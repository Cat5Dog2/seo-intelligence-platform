...
ログイン導線は「登録不要でデモを試す」。ゲスト案内は「設定・アカウント」内の折りたたみ欄にまとめ、ヘッダーにはゲスト・Mockのバッジ、各サンプル画面には展開できる閲覧専用の案内を表示する。キーワード探索の詳細条件は通常ログインのみ編集できる。検索ボリューム調査は開始後に結果を自動表示し、候補語・検索ボリュームのCSV受付後は出力一覧が自動で更新される。
