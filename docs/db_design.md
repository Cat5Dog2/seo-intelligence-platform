...
| テーブル | 主なカラム | 制約/備考 |
| --- | --- | --- |
| `locations` | `id uuid PK`, `provider text`, `location_code text`, `location_name text`, `country_code text`, `status text`, `synced_at timestamptz` | 地域マスタ。`UNIQUE(provider, location_code)`。ラッコキーワードAPI v1.12.0以降はcode値が提供されないため、`location_code`にはAPIリクエストで使う地域名を正準値として格納する。 |
| `languages` | `id uuid PK`, `provider text`, `language_code text`, `language_name text`, `status text`, `synced_at timestamptz` | 言語マスタ。`UNIQUE(provider, language_code)`。ラッコキーワードAPI v1.12.0以降はcode値が提供されないため、`language_code`にはAPIリクエストで使う言語名を正準値として格納する。 |
