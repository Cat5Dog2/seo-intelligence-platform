...
- サイドメニューを「調査・分析」「コンテンツ」「監視・レポート」「管理」に分ける。プロジェクトとダッシュボードは先頭に置く。幅920px以下では開閉式メニューにし、遷移時に閉じる。
...
ゲストのヘッダーには「ゲスト・Mock」、本文先頭には操作範囲と初期化条件を表示する。プロジェクト・サイト編集、キーワード探索、検索ボリューム調査、候補語/検索ボリュームのCSV出力を操作できる。競合、獲得語/ページ、コンテンツ分析、クラスター、記事ブリーフ、順位、リライトは固定サンプルを閲覧できる。その他の生成・更新、管理設定、通知、AI、共有URL発行はデモ対象外とし、実行時に日本語で案内する。詳細は`guest_login.md`を参照する。
