...
**デプロイは単一のUnixユーザーで行うこと。** ロックは`XDG_RUNTIME_DIR`（ユーザー専用0700）または`/var/lock`に置くため、排他されるのは同一ユーザーの操作だけである。複数ユーザーでデプロイする運用にする場合は、そのユーザーだけが書ける専用ディレクトリを事前に作成し、`PRODUCTION_LOCK_DIR`で指定する。安全な置き場所が決まらない場合、スクリプトは`/tmp`へフォールバックせず**実行を拒否する**（誰でも保持できるロックはロックではないため）。
