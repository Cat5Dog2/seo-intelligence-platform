...
| `container-scan` | PostgreSQL/Redisを `image-digests.lock` のdigest（本番が起動するイメージ）でpullしてTrivyでvuln-only scanし、ゲートする。上流タグの先でlinux/amd64のimageが変わったかは報告のみ（warning annotation）で、開発専用RustFSも報告のみとする。詳細は `docs/operations_runbook.md` 7.3節。 |
