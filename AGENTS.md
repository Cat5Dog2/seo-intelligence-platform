...
- 本番コードに `if (isTestMode)` や `#if DEBUG` のようなテスト都合の分岐を入れない
- テスト用の特別な値（マジックナンバー）を本番コードに埋め込まない
- 環境差は appsettings、Options pattern、DI で分離すること

#### テスト実装の原則
- バグ修正と新規ビジネスロジックは、失敗するテストから始めること（Red-Green-Refactor）。リファクタ、配線、設定変更は対象外
