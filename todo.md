...
- [x] `PRODUCTION_LOCK_DIR`の検証を追加した。従来は指定されたディレクトリをそのまま使っていたため、他ユーザーが先回りして作成したディレクトリでも受け入れていた。`readlink -f`で解決した上で、実行ユーザーの所有かつother書き込み不可の既存ディレクトリであることを要求し、スクリプト側では作成しない。
- [x] 再入検証を継承FD自体のロックへ変更した。パスを開き直す実装は「誰かが保持している」ことしか示さないため、他プロセスが保持している最中に`PRODUCTION_LOCK_HELD`を立てればロックを迂回できた。Linuxで、偽装が拒否され正規の子プロセスだけが再入できることを確認済み。
- [x] 別cwdテストの入力を、正しい実装でも退行した実装でも`$work`配下へ落ちる相対パスへ変更した。従来は退行を実証するたびにリポジトリへ成果物が残った。退行注入時に残置がないことを確認済み。
...
- ゲートが答えるべき問いは「本番で動いているイメージに未判断の修正可能な HIGH/CRITICAL があるか」であり、「上流タグが動いたか」ではない。未受容の HIGH/CRITICAL は、Trivy の脆弱性DBに FixedVersion が載ればピンの検査で赤になる。受容済みCVEは修正版が公開されても除外が続くため、drift の報告を契機に手動レビューして新digestの採用を判断する。タグの移動は更新リマインダーであってセキュリティ失敗ではない。
...
参照ドキュメント: `docs/api_design.md`, `docs/screen_design.md`, `docs/test_plan.md`

- [x] QA-01: ダッシュボードのDTO射影を並べ替え後に行い、PostgreSQLのSQL変換エラーを修正する。
- [x] QA-02: クラスター/コンテンツ分析からのブリーフ生成でキーワードIDと文字列を同時送信しない。
- [x] QA-03: AI許可ツールの既定値をAPIでサポートする値へ修正する。
- [x] QA-04: プロジェクトスコープ付きAI応答取得APIと画面の更新/履歴参照導線を追加する。
- [x] QA-05: `csv_export`のダウンロードリンクと出力元画面のジョブ一覧を追加する。
- [x] QA-06: 非同期キーワード探索の完了結果を保存・取得し、「状態更新」で候補語とソース状態を反映する。
- [x] 不具合を再現するテストを先に実行し、修正後にRazor/API/PostgreSQLの関連テストを確認する。
